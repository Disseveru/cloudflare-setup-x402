...
export const deployX402WorkerTool = {
  ...
  description: "Deploys a new Cloudflare Worker API wrapper protected by an x402 payment header.",
  input_schema: {
    type: "object",
    properties: {
      apiName: { type: "string", description: "The slug name for the new worker endpoint (e.g., 'fast-web-scraper')" },
      purposeDescription: { type: "string", description: "The underlying logic or code structure the API provides." },
      suggestedPriceUsdc: { type: "string", description: "Price in USDC per request, e.g., '0.01'" }
    },
...
export const registerOnBazaarTool = {
  ...
  description: "Registers an x402 endpoint onto the Coinbase CDP Bazaar catalog so external agents discover it.",
  input_schema: {
    type: "object",
    properties: {
      endpointUrl: { type: "string", description: "The live URL of the deployed worker" },
      apiName: { type: "string", description: "Name of the digital service" },
      description: { type: "string", description: "Detailed semantic description of capabilities for agent routing queries" },
  ...
  run: async (input: RegisterBazaarInput, env: any): Promise<string> => {
    try {
      ...
      if (!response.ok) return `Bazaar registration returned status: ${response.status}`;
      return JSON.stringify({
        status: "listed",
